...
bool CollisionObject::checkCollision(CollisionObjectPtr other) {
    ...
    oMax.y += fmin((position.x - oMin.x) / (oMax.x - oMin.x) * (other->shearX2 - other->shearX1) + other->shearX1, oMax.y - oMin.y);
    oMax.y += fmin((position.z - oMin.z) / (oMax.z - oMin.z) * (other->shearZ2 - other->shearZ1) + other->shearZ1, oMax.y - oMin.y);
