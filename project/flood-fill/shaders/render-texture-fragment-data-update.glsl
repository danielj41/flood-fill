...
void main(){
    ...
    //height += 5.0 * uDTime.x / (uDTime.y + 0.1) / uSize.y / (dot(coord - increasePosition, coord - increasePosition) + 0.01);

    //height = max(height, (1.0 - amount) * (block.r + 0.01/uSize.y));

    //height = (1.0 - amount) * info.r + amount * (1.0 / uSize.y);
