...
void MainLevel::setup(){
    ...
    CameraPtr cam1(new Camera(glm::vec3(0, 1, 0), glm::vec3(0, 0, -5),
                             glm::vec3(0, 1, 0)));
    ...
    GroundPlanePtr ground(new GroundPlane());
    ground->setup();
    ...
    InvisibleWallPtr wall1(InvisibleWall(glm::vec3(-30, 0, 0),
                                              glm::vec3(0.5, 10, 30)));
    wall1->setup();
    addGameObject("wall1", wall1);
    CollisionManager::addCollisionObject(wall1);

    InvisibleWallPtr wall2(new InvisibleWall(glm::vec3(30, 0, 0),
                                              glm::vec3(0.5, 10, 30)));
    wall2->setup();
    addGameObject("wall2", wall2);
    CollisionManager::addCollisionObject(wall2);

    InvisibleWallPtr wall3(new InvisibleWall(glm::vec3(0, 0, 30),
                                              glm::vec3(30, 10, 0.5)));
    wall3->setup();
    addGameObject("wall3", wall3);
    CollisionManager::addCollisionObject(wall3);

    InvisibleWallPtr wall4(new InvisibleWall(glm::vec3(0, 0, -30),
                                              glm::vec3(30, 10, 0.5)));
    wall4->setup();
